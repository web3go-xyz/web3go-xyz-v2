...
class Component extends React.Component {
                                    ...
                                    <div className="tag-list">
                                        {
                                            record.tagList.map(v => (
                                                <div key={v.id} title={v.tagDescription} className="item hover-item" onClick={() => { this.handleClickTab2(v) }}>{v.tagName}</div>
                                            ))
                                        }
                                    </div>
    ...
    handleClickTab = (v) => {
        const newCurrentTagList = [...this.state.currentTagList];
        const findIndex = this.state.currentTagList.findIndex(sv => sv.id == v.id)
        if (findIndex !== -1) {
            newCurrentTagList.splice(findIndex, 1)
        } else {
            newCurrentTagList.push(v);
        }
        this.changeCurrentTagList(newCurrentTagList)
    }
    handleClickTab2 = (v) => {
        const newCurrentTagList = [...this.state.currentTagList];
        const findIndex = this.state.currentTagList.findIndex(sv => sv.id == v.id)
        ...
        return (
            <div className="web3go-layout-home-dashbaoard-list">
                <div className="filter-wrap">
                    <div className={cx("filter-btn", { green: this.state.currentTagList.length })} onClick={() => { this.clickFilter(); }}>
                    ...
                    </div>
                    {
                        !this.state.paramsShow ? (
                            <div className="filter-list">
                                <div
                                    className={"item" + (!this.state.currentTagList.length ? ' active' : '')}
                                    onClick={() => { this.changeCurrentTagList([]) }}
                                    title="All"
                                >All</div>
                                {
                                    this.state.filterList.slice(0, 10).map((v, i) =>
                                        <div
                                            className={"item" + (this.state.currentTagList.find(sv => sv.id == v.id) ? ' active' : '')}
                                            onClick={() => { this.handleClickTab(v) }}
                                            title={v.tagName}
                                            key={i}>{v.tagName}</div>
                                    )
                                }
                            ...
                            <div className="search-params-wrap">
                                <div className="Dashboard-tag">
                                    <Select
                                        ...
                                        style={{ width: 386 }}
                                        allowClear
                                        value={this.state.currentTagList.map(v => v.id)}
                                        mode="multiple"
                                        onChange={(value) => {
                                            const currentTagList = this.state.filterList.filter(v => value.includes(v.id));
                                            this.setState({
                                                currentTagList
                                            }, () => {
                                                this.getList(true);
