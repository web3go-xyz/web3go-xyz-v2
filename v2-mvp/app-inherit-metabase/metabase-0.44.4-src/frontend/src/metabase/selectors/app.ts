...
export const getIsWeb3go = createSelector([getRouterPath], path => {
...
export const getIsNavBarVisible = createSelector(
  ...
  (currentUser, path, isEditingDashboard, isEmbedded, embedOptions, isWeb3go) => {
    if (!currentUser || isEditingDashboard || isWeb3go) {
      return false;
    }
